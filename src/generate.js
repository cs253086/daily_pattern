...
=== UNIT-LEVEL DYNAMICS (HARD — measured and rejected if the pattern only moves as one rigid block) ===
The channel's owner defines "dynamic" precisely: the BASIC UNITS of the pattern (the tiles, cells, bands, rings, solids, segments, whatever the pattern is built from) must visibly move RELATIVE TO EACH OTHER every second — pulsing, swaying, orbiting, breathing, counter-rotating, morphing — not merely the whole arrangement spinning or scrolling as one rigid piece. An automated check fits one rigid rotation/translation of the whole frame between two frames a quarter second apart; if that single transform explains most of the change, the engine is rejected as lifeless. Concretely, add at least two of these, big enough to be obvious at a glance (10-35 percent of a unit's size, cycle periods of 3-8 seconds, every rate with a guaranteed minimum magnitude and random sign):
- Travelling waves across units keyed to position: phase equals wave-vector dot position minus omega times time, plus a little per-unit jitter, modulating each unit's scale about its own centre, its offset along a radial or tangential direction, or its own rotation. Because unit phases are spread evenly, total lit coverage stays constant at every instant, which keeps the brightness-trend rule below happy.
- Per-unit spin: each unit rotates about its own centroid at its own rate.
- Counter-rotation: concentric rings, tiers or layers rotating at clearly different rates and directions.
- Continuous morphing of the generating parameters (seed points drifting, curve parameters oscillating within a band, branch angles swaying with depth), recomputing geometry every frame when it is cheap enough for the performance budget below.
Whole-pattern rotation is welcome on top of this, never instead of it.

=== EVOLVE OVER THE HOUR (HARD — measured and rejected if the composition is frozen) ===
Movement is not the same thing as change, and this video runs for a FULL HOUR. An engine that builds one arrangement at startup and then only spins, scrolls, or orbits it forever passes every motion check while showing the viewer effectively the same picture from minute 2 to minute 60. That is the single most common way an otherwise good engine becomes boring, and it is now automatically detected: the validator measures your composition with a ROTATION-INVARIANT descriptor at several points across the timeline, so a rigidly-spinning fixed layout scores near zero and is REJECTED no matter how fast it spins.
What is required: the ARRANGEMENT ITSELF must keep developing — over tens of seconds and minutes, new structure should appear, existing structure should reorganise, and the frame a viewer sees at minute 30 should be recognisably a different composition from the one at minute 1 (same style and palette family, different arrangement).
The catch, and how to satisfy BOTH this and the brightness rules above: the easy way to make things change is to re-randomise the scene every cycle, but anything that changes how MUCH of the frame is covered/overlapping between cycles creates a real frame-average brightness swing, which the whiteout trend check will reject. So make the composition evolve in ways that hold total coverage roughly CONSTANT. Reliable techniques, any of which is enough:
1. Constant-count morphing. Fix the NUMBER and SIZE of elements once, then continuously move/reshape them — interpolate every element toward fresh target positions/parameters, reach them, pick new targets, repeat. Element count and size never change, so coverage cannot drift, but the layout is never the same twice.
2. Rank-select a fixed fraction. Compute some scalar field or score per candidate element every frame, then draw exactly the top N (or lowest N) of them. The number drawn is fixed by construction, so the underlying field can morph completely freely — including changing shape entirely — with zero coverage change. This is the most robust option when the underlying pattern is generated from a formula.
3. Cross-fade between two fixed configurations. Build configuration A and configuration B once at startup with the SAME element count and sizes, then continuously blend every element's position/shape from A toward B and back (or on to a fresh C). Every intermediate state has identical coverage.
4. Travel through a larger field. Generate a field substantially bigger than the visible frame and pan/drift the viewpoint slowly across it, so genuinely new material enters the frame continuously. Keep the field statistically uniform so the amount visible stays constant, and prefer panning over zooming (zooming changes how much is on screen).
5. Continuous element turnover at fixed population. Retire one element and introduce a new one somewhere else at the same time, keeping the live count pinned.
Do NOT attempt to satisfy this by re-rolling the whole scene on a timer, by fading the palette (hue changes brightness even at fixed saturation, which the trend check reads as drift), or by simply spinning faster.
...
4. Declare every top-level helper (the 4x4 matrix helper, shader source strings, buffer/program variables) with its full definition BEFORE any code that calls or uses it runs -- do not call an initialization function and then declare a const/let it depends on further down the file. A real generated engine has failed this exact way: "ReferenceError: Cannot access 'mat4' before initialization", from a matrix helper declared with const AFTER the init function that used it was already being invoked. If you structure the file as helper declarations first, then an init() function, then a single call to init() at the very end, this class of bug cannot happen.
...
GLOW IS MANDATORY, NOT OPTIONAL FLOURISH: research into what makes ambient/screensaver pattern videos actually watched (the channels with the largest audiences in this exact genre) found soft bloom/glow is the single biggest visual difference between a flat "programmer art" render and a premium one — more than subject matter. Every bright shape/line in this engine MUST read as glowing, not flat-filled. The reliable, zero-risk way to do it in Canvas2D: draw every bright element TWICE in the same frame — once with a wide, low-alpha, blurred-looking stroke (either a real 'ctx.shadowBlur'/'ctx.shadowColor' glow, or several progressively-larger, progressively-more-transparent copies of the same stroke) UNDERNEATH, then a thin, near-full-opacity core stroke on top. This costs nothing across frames (no accumulation, so it cannot cause the whiteout failure mode described above) and reads as genuinely luminous instead of a flat vector line. Do this for every significant shape, not just a decorative highlight or two.
